...
class IMDB:
    def __init__(self, id: str, lang: str, html: BeautifulSoup | None = None):
        self.id = id
        self._lang = lang
        self._html = html
    ...
    async def get_related_media(self, ids_only: bool = False) -> list[IMDB]:
        ...
        for media_id in media_ids:
            tasks.append(get_media(media_id, self._lang))
...
async def get_media(
    ...
    return IMDB(id, lang, html=imdb_html)


async def search(term: str, lang: str, ids_only: bool = False) -> list[IMDB] | list[str]:
    ...
    # return IMDB objects of the results
        ...
        for result in results:
            ...
            if re.match(r"^tt\d+$", id):
                if result["qid"] in ("movie", "tvSeries", "tvMiniSeries") and "y" in result.keys():
                    tasks.append(get_media(id, lang))
